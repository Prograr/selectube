...
class YoutubeHelper extends HtmlHelper {
    ...
    function thumbnail($url, $size = 'thumb', $options = array()) {
        ...
        $image_url = sprintf(
                $this->apis['image'], $video_id, $accepted_sizes[$size]);
    ...
    function video($url, $settings = array(), $variables = array()) {
        ...
        return $this->tag('object', $this->tag('param', null, array('name' => 'movie', 'value' => $variables['src'])) .
    ...
    function getVideoId($url = null) {
        ...
        //JS
        if (preg_match("@youtube.com/@", $url)) { //http://www.youtube.com/watch?v=GItD10Joaa0
            
            preg_match('@[\\?&]v=([^&#]*)@', $url, $results);
            
//            $id = $results[1];
            
            return ($results === null) ? $url : $results[1];
        } else if (preg_match("@^http://youtu.be/.*@", $url)) { // http://youtu.be/5uxd-521uus?hd=1
            
            preg_match('@^http://youtu.be/(.*)@', $url, $results);
